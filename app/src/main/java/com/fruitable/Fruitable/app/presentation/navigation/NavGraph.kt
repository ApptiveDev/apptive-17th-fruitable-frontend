...
fun NavGraphBuilder.fruitableGraph(
    ...
    composable(route = Screen.UserInfoUpdateScreen.route) { UserInfoUpdateScreen(navController) }
    /**
     * setting - 설정, 계정 정보, 이메일 정보, 판매자 등업, 회원 탈퇴, 공지, 공지 상세 Screen
     */
    composable(route = Screen.AccountScreen.route) { AccountScreen(navController) }
    composable(route = Screen.SettingScreen.route) { SettingScreen(navController) }
    composable(route = Screen.UpgradingScreen.route) { UpgradingScreen(navController) }
    composable(route = Screen.EmailScreen.route) { EmailScreen(navController) }
    composable(route = Screen.LeaveAppScreen.route) { LeaveAppScreen(navController) }
    composable(route = Screen.NoticeScreen.route) { NoticeScreen(navController) }
    composable(
